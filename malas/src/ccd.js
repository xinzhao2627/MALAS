...
export default function CCD({colorData, ccdProceed, setCcdProceed}) {
    const ret_data = colorData
    const [colors, setColors] = useState(Array(3).fill("turquoise"));
    const [inputs, setInputs] = useState(Array.from({ length: 3 }, (x, i) => 'CCD at ' + (i + 1)));
